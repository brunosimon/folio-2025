...
export class VisualVehicle {
  ...
  setParts() {
    ...
    if (this.parts.stopLights) {
      ...
      this.parts.stopLights.baseMaterial = this.parts.stopLights.material;
      this.parts.stopLights.reverseMaterial = new THREE.MeshBasicMaterial({
        color: new THREE.Color(0xffffff).multiplyScalar(7.75),
      });
    }
  ...
  setPaints() {
    ...
    // Abyssal
    {
      ...
      material.outputNode = Fn(() => {
        ...
        const starsColor = texture(
          this.game.resources.behindTheSceneStarsTexture,
          starsUv
        )
          .rgb.pow(2)
          .mul(starsIntensity);
  ...
  setBlinkers() {
    ...
    const start = () => {
      ...
      this.parts.blinkerLeft.visible = this.game.inputs.actions.get("left")
        .active
        ? on
        : false;
      this.parts.blinkerRight.visible = this.game.inputs.actions.get("right")
        .active
        ? on
        : false;
    ...
    const blink = () => {
      ...
      if (
        !this.game.inputs.actions.get("left").active &&
        !this.game.inputs.actions.get("right").active &&
        !on
      ) {
        running = false;
      } else {
  ...
  setAntenna() {
    ...
    this.antenna.head =
      this.game.resources.vehicle.scene.getObjectByName("antennaHead");
    this.antenna.headAxle = this.antenna.head.children[0];
    this.antenna.headReference = this.antenna.object.getObjectByName(
  ...
  setBoostTrails() {
    ...
    this.boostTrails.leftReference.getWorldPosition(
      this.boostTrails.left.position
    );
    ...
    this.boostTrails.rightReference.getWorldPosition(
      this.boostTrails.right.position
    );
  }
  ...
  setBoostAnimation() {
    ...
    if (this.parts.energy) {
      const emissiveOuput = this.game.materials.getFromName(
        "emissivePurpleRadialGradient"
      ).outputNode;
      const defaultOutput = this.parts.energy.material.outputNode;

      const material = new THREE.MeshLambertNodeMaterial();
      material.outputNode = mix(
        defaultOutput,
        emissiveOuput,
        this.boostAnimation.mixUniform
      );
  ...
  update() {
    ...
    this.wheels.steering +=
      (this.game.player.steering * physicalVehicle.steeringAmplitude -
        this.wheels.steering) *
      this.game.ticker.deltaScaled *
      16;

    const wheelsRotation =
      (physicalVehicle.forwardSpeed / physicalVehicle.wheels.settings.radius) *
      0.006;
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (visualWheel.suspension) {
        const suspensionScale =
          Math.abs(visualWheel.container.position.y) - 0.5;
        visualWheel.suspension.scale.y = suspensionScale;
      }

      // Ground tracks
      visualWheel.groundTrack.update(
        physicalWheel.contactPoint,
        physicalWheel.inContact
      );
    }
    ...
    if (this.antenna) {
      ...
      const antennaTargetDistance = this.antenna.target.distanceTo(
        physicalVehicle.position
      );

      const antennaRotationSpeed = remapClamp(
        antennaTargetDistance,
        50,
        5,
        1,
        10
      );
      this.antenna.headAxle.rotation.z +=
        this.game.ticker.deltaScaled * antennaRotationSpeed;
    }

    // Stop lights
    const isPlayerReverseBraking =
      this.game.physicalVehicle.speed *
        Number(this.game.physicalVehicle.goingForward) >
        0 && this.game.inputs.actions.get("backward").active;

    const isPlayerReversing = Number(this.game.physicalVehicle.goingForward) === 0 && this.game.inputs.actions.get("backward").active;

    if (this.game.player.braking || isPlayerReverseBraking) {
      this.parts.stopLights.visible = true;
      this.parts.stopLights.material = this.parts.stopLights.baseMaterial;
    } else if (isPlayerReversing) {
      this.parts.stopLights.visible = true;
      this.parts.stopLights.material = this.parts.stopLights.reverseMaterial;
    } else {
      this.parts.stopLights.visible = false;
    }
    ...
    this.boostAnimation.mix +=
      (this.game.player.boosting ? 1 : -1) *
      this.game.ticker.deltaScaled *
      this.boostAnimation.speed;
    ...
    this.boostAnimation.mixUniform.value =
      1 - Math.pow(1 - this.boostAnimation.mix, 7);
    if (this.parts.energy) {
      this.parts.cell1.position.y = remapClamp(
        this.boostAnimation.mix,
        0,
        0.6,
        0.2,
      ...
      this.parts.cell3.position.y = remapClamp(
        this.boostAnimation.mix,
        0.2,
        0.8,
        0.2,
      ...
      this.parts.cell2.position.y = remapClamp(
        this.boostAnimation.mix,
        0.4,
        1,
        0.2,
